...
Create a meeting with a `HTTP request` containing the `API_KEY` sent to videodialling’s server. The response contains a `meeting` URL that can be `embedded` in your client within an `iframe`.
...
<iframe
    ...
    src="https://sfu.videodialling.com/join/your_room_name"
